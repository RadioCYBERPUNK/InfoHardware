...
def get_hardware_info():
    try:
        ...
        # Informações da Placa de Vídeo NVIDIA
        print("Informações da Placa de Vídeo NVIDIA:")
        pynvml.nvmlInit()
        ...
        for i in range(device_count):
            ...
            print(f"Placa de Vídeo {i + 1}: {name}")

            memory_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
            print(f"Memória Total: {memory_info.total} bytes")
            print(f"Memória Usada: {memory_info.used} bytes")
            print(f"Memória Livre: {memory_info.free} bytes")

            driver_version = pynvml.nvmlSystemGetDriverVersion()
            print(f"Versão do Driver: {driver_version}")
